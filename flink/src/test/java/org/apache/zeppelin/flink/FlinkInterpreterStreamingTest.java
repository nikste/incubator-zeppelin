...
public class FlinkInterpreterStreamingTest {
  ...
  public void testWordCountCollect() {
    System.out.println(flink.interpret("val text = env.fromElements(\"To be or not to be\")", context).message());
    System.out.println(flink.interpret("val counts = text.flatMap { _.toLowerCase.split(\"\\\\W+\") filter { _.nonEmpty } }.map { (_, 1) }.groupBy(0).sum(1)", context).message());
